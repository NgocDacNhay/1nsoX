...
//            this.sendXYPlayer(p);
    ...
    public void attackAMob(@Nullable final Body body, @Nullable Mob curMob, int dame) throws IOException {
        ...
        if (this.map.cave == null && curMob.isIsboss() && body.getLevel() - curMob.level > 50) {
            dame = 0;
        }
...
//            }
    ...
    protected boolean canAttackNinja(final @Nullable Body body, final @Nullable Ninja other) {
        ...
        User p1 = body.c.p; 
        if(body.getTypepk() == PK_DOSAT && other.ItemBody[10] != null && other.ItemBody[10].id == 744){
            p1.sendYellowMessage("Có dame đâu mà đánh");
            return false;
        }
    ...
    public void attackNinja(final  Body body, Message m) throws IOException {
        if (body == null || m == null) {
            return;
        }
        val p = body.c.p;
