...
public class User extends Actor implements SendMessage {
    ...
    public boolean isDiemDanh = false;
    ...
    public static User login(final Session conn, final String user, final String pass) {
        ...
        SQLManager.executeQuery(query, (red) -> {
            if (red != null && red.first()) {
                ...
                final boolean isDiemDanh = red.getBoolean("DiemDanh");
                final boolean isNewAccount = red.getBoolean("newAccount");
                final long ngoc = red.getInt("VND");
                ...
                p.luong = luong;
                p.passold = pass;
                p.isDiemDanh = isDiemDanh;
                p.isNewAccount = isNewAccount;
                p.Ngoc = ngoc;
                p.vip = vip;
                if(p.vip >= 2){
                    p.isSvip = true;
...
//            restPoint();
        ...
        m.writer().writeShort(nj.partHead());
        m.writer().writeShort(nj.Weapon());
        m.writer().writeShort(nj.partBody());
        m.writer().writeShort(nj.partLeg());
        for (byte i = 0; i < nj.setThoiTrang.length; i++) {
            m.writer().writeShort(nj.setThoiTrang[i]);
        }
        ...
        if (enter) {
            ...
            for (int i = 0; i < maps.length; i++) {
                ...
                if (map.id == this.nj.getMapid()) {
                    ...
                    if (map.getXHD() != -1 || map.VDMQ() || map.isGtcMap() || map.isLdgtMap() || map.isChienTruongKeo() || map.isLoiDai() || map.isNvMap() || map.id == 112 || map.id == 113) {
                        isturn = true;
...
//        conn.disconnect();
    ...
    @SneakyThrows
    public synchronized void restSpoint() throws IOException {
        if (lastTimeResetPoint != -1 && System.currentTimeMillis() - lastTimeResetPoint < MIN_TIME_RESET_POINT) {
            return;
        }
        ...
        for (final Skill skill : this.nj.get().getSkills()) {
            if (skill.id != 0) {
                if (nj.isHuman && nj.get().checkIdSkill90(skill.id)) {
                    continue;
                }
                skill.point = 1;
            }
            ...
            for (byte i = 0; i < this.sortNinja.length; ++i) {
                if (name.equals(this.sortNinja[i])) {
                    ...
                    if (this.nj != null) {
                        ...
                        SQLManager.executeQuery("SELECT `vip` FROM `player` WHERE `id`=" + this.id + " LIMIT 1;", (res) -> {
       if (res != null && res.first()) {
                                vip = res.getInt("vip");
                                if (vip >= 3) {
                                    Manager.chatKTG("Chào mừng IDOL " + this.nj.name +" đã đăng nhập vào game.");
                        }
                            }
                            res.close();
                        });
                        ...
                        SQLManager.executeQuery("SELECT `content` FROM `content` WHERE `id` = 1 LIMIT 1;", (res) -> {
                            String content;
                            String contentExpCuoiTuan;
                            String getDay = util.dateFormatDay.format(Date.from(Instant.now()));
                            int dd = Integer.parseInt(getDay.substring(0, 2));
                            String MM_yyyy = getDay.substring(2);
                            int mm = Integer.parseInt(MM_yyyy.substring(1, 3));
                            int yyyy = Integer.parseInt(MM_yyyy.substring(4));
                            switch (util.getDay(Date.from(Instant.now()))) {
                                case "Saturday": {
                                    ...
                                    dd += 1;
                                    contentExpCuoiTuan = "- X2 kinh nghiệm đến hết ngày " + dd + "/" + mm + "/" + yyyy;
                                    break;
                                }
                                case "Sunday": {
                                    contentExpCuoiTuan = "- X2 kinh nghiệm đến hết ngày " + dd + "/" + mm + "/" + yyyy;
                            ...
                            if (res != null && res.first()) {
                                ...
                                server.manager.sendTB(this, "Server", content + "\n" + contentExpCuoiTuan);
                            }
                            res.close();
                        });
...
// Tắt bẬT Tạo nhân vật
    ...
    public void useItem(final Message m) throws IOException, InterruptedException {
    ...
    //    }
        ...
        if (item.id == 194) {
            this.session.sendMessageLog("Hiện tại không thể bán, vui lòng hoàn thành nhiệm vụ tiếp theo!");
            return;
        }
    ...
    public void itemBodyToBag(Message m) throws IOException {
        ...
        for (int k = 0; k < 32; ++k) {//Trang bị 2
            ...
            if (item != null) {
                if (this.nj.isNhanban) {
                    ThoiTrang.setThoiTrangPT(this.nj.clone, item.id);
                } else if (this.nj.isHuman) {
                    ThoiTrang.setThoiTrang(this.nj, item.id);
                }
            }
        }
        if (itembody.id == 568) {
            ...
            if (p != null && p.nj != null && p.session != null && m != null && m.reader().available() > 0) {
                ...
                switch (p.menuCaiTrang) {
                    ...
                    case 4: {
                        ...
                        if (data.type == 33) {
                            if (p.nj.quantityItemyTotal(837) > 0) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        im.expires += 2592000000L;
                                        p.nj.removeItemBags(837, 1);
                            ...
                            if (p.nj.quantityItemyTotal(836) > 0) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        im.expires += 2592000000L;
                                        p.nj.removeItemBags(836, 1);
                                        p.session.sendMessageLog("Gia hạn thành công " + ItemData.ItemDataId(im.id).name + " thêm 30 ngày");
                                    } else {
                                        p.session.sendMessageLog("Vật phẩm không có hạn sử dụng");
                                    }
                            ...
                            if (p.nj.quantityItemyTotal(835) > 0) {
                                if (im != null) {
                                    if (im.isExpires == true) {
                                        im.expires += 2592000000L;
                                        p.nj.removeItemBags(835, 1);
                                        p.session.sendMessageLog("Gia hạn thành công " + ItemData.ItemDataId(im.id).name + " thêm 30 ngày");
                                    } else {
                                        p.session.sendMessageLog("Vật phẩm không có hạn sử dụng");
                                    }
...
//    }
            ...
            if (item.option != null) {
                ...
                if (item.ngocs != null && item.ngocs.size() > 0) {
                    ...
                    if (item.getData().type == 1) {
                        idNgoc = ItemData.VU_KHI_OPTION_ID;
                    } else if (item.getData().isTrangSuc()) {
                        idNgoc = ItemData.TRANG_SUC_OPTION_ID;
                    } else if (item.getData().isTrangPhuc()) {
                        idNgoc = ItemData.TRANG_BI_OPTION_ID;
                    }

                    if (idNgoc != -1) {
                        for(Item itemNgoc : item.ngocs) {
                            for(Option op : itemNgoc.option) {
                                if(op.id == idNgoc) {
                                    int indx = itemNgoc.option.indexOf(op);
                                    if(indx != -1) {
                                        if (itemNgoc.id == ItemData.HUYEN_TINH_NGOC) {
                                            m.writer().writeByte(109);
                                        } else if (itemNgoc.id == ItemData.HUYET_NGOC) {
                                            m.writer().writeByte(110);
                                        } else if (itemNgoc.id == ItemData.LAM_TINH_NGOC) {
                                            m.writer().writeByte(111);
                                        } else if (itemNgoc.id == ItemData.LUC_NGOC) {
                                            m.writer().writeByte(112);
                                        }
                                        Option op1 = itemNgoc.option.get(indx + 1);
                                        Option op2 = itemNgoc.option.get(indx + 2);
                                        m.writer().writeInt(0);
    ...
    public void requestItemInfo(final Message m) throws IOException {
        ...
        try {
            ...
            if(p != null && p.nj != null && p.session != null && !p.nj.isDie && m != null && m.reader().available() > 0){
    ...
    public void requestTrade(Message m) throws IOException {
        if (this.nj.getLevel() < 20) {
            this.session.sendMessageLog("Yêu cầu trình độ cấp 20");
            return;
        }
        ...
        if (this.nj.isNhanban) {
            this.session.sendMessageLog("Chức năng không dành cho phân thân");
            return;
        }
        ...
        if (p == null) {
        ...
        } else if (p.nj.tradeDelay > System.currentTimeMillis()) {
            ...
            this.nj.tradeDelay = System.currentTimeMillis() + 15000L;
        ...
        if (this.nj.tradeLock == 1) {
            ...
            if (n.tradeLock == 2) {
                ...
                if (n.tradeCoin > 0) {
                    ...
                    this.LogHistory.log(String.format(c.name + " đã giao dịch với " + n.name + " và nhận được: " + n.tradeCoin + " xu."));
                }
                if (this.nj.tradeCoin > 0) {
                    this.nj.upxuMessage(-this.nj.tradeCoin);
                    n.upxuMessage(this.nj.tradeCoin);
                    this.LogHistory.log2(String.format(n.name + " đã giao dịch với " + c.name + " và nhận được: " + c.tradeCoin + " xu."));
                }
    ...
    public void flush() {
       ...
       //     SQLManager.executeUpdate("UPDATE `player` SET `quavip`=" + this.quavip + ",`ninja`='" + jarr.toJSONString() + "' WHERE `id`=" + this.id + " LIMIT 1;");
            ...
            SQLManager.executeUpdate("UPDATE `player` SET `DiemDanh`=" + this.isDiemDanh + " WHERE `id`=" + this.id + " LIMIT 1;");
            SQLManager.executeUpdate("UPDATE `player` SET `newAccount`=" + this.isNewAccount + " WHERE `id`=" + this.id + " LIMIT 1;");
    ...
    public synchronized void updateExp(long xpup, boolean useMulti) throws IOException {
        ...
        if (this.nj.get().expdown > 0L) {
            ...
            if (oldLv < this.nj.get().getLevel()) {
                try {
                    SQLManager.executeUpdate("UPDATE ninja SET timeUpLevel='" + util.toDateString(Date.from(Instant.now())) + "' WHERE `name`='" + this.nj.name + "' LIMIT 1");
                } catch (SQLException ex) {
                    ex.printStackTrace();
                }
                if (this.nj.get().nclass != 0) {
                    ...
                    if (op.id == 65) {
                        ...
                        for (byte j = 0; j < useItem.arrOp.length; ++j) {
                            if (useItem.arrOp[j] == op.id) {
                                ...
                                option.param += useItem.arrParam[j];
                                break;
                            }
                        }
                    }
    ...
    public boolean updateXpMounts(final int xpup, final byte type) {
        ...
        if (type == 0 && item.id != 443 && item.id != 523 && item.id != 524 && item.id != 801 && item.id != 802 && item.id != 803 && item.id != 831) {
        ...
        if (item.getUpgrade() < 99) {
            ...
            while (i < item.option.size()) {
                ...
                if (op.id == 65) {
                    ...
                    if (op.param >= 1000) {
                        isuplv = true;
                        if (item.getUpgrade() == 98) {
                            op.param = 0;
                        } else {
                            op.param = op.param - 1000;
                        }
                        break;
                    }
                    break;
                        ...
                        if (op2.id != 65 && op2.id != 66) {
                            for (byte k = 0; k < useItem.arrOp.length; ++k) {
                                if (useItem.arrOp[k] == op2.id) {
                                    ...
                                    option2.param += useItem.arrParam[k] / 5 * 2;
                                    break;
                                }
                            }
                        }
    ...
    public void sendRequestBattleToAnother(Ninja friendNinja, Ninja _n) {
        Message m = null;
        try {
            m = new Message(-157);
            m.writer().writeInt(_n.id);
            friendNinja.p.session.sendMessage(m);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            if (m != null) {
    ...
    public void CanCau() throws IOException {
        Item itemup;
    ...
    public void isSell(String name1, String itemsell, int slsell) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            this.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = util.toDateString(Date.from(Instant.now()));
            String filename = "His_Log_Game/HisSell/Sell_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("Name : " + name1 + " - Time : " + str + " - ItemSell :" + itemsell + " - Quantity :" + slsell + "\n");
            bw.close();
            fw.close();
        } catch (Exception e) {
    ...
    public void isBuy(String name1, Short itembuy, int yen, int xu, int luong, int luongsau, int luongtruoc) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            this.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = util.toDateString(Date.from(Instant.now()));
            String filename = "His_Log_Game/HisBuy/Buy_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("Name : " + name1 + " - Time : " + str + " - ItemBuy :" + itembuy + " - Yên mua :" + yen + " - Xu mua :" + xu + " - Lượng mua :" + luong + " - Lượng trước mua :" + luongtruoc + " || Lượng sau mua :" + luongsau + "\n");
            bw.close();
            fw.close();
        } catch (Exception e) {
            System.out.println(e);
    ...
    public void isGiaoDich(String name1, String name2, String itemsend, int xusend, String itemadd, int xuadd) {
        try {
            Calendar calender = Calendar.getInstance();
            Date date = calender.getTime();
            this.dateFormatFile = new SimpleDateFormat(DATE_FORMAT_FILE);
            String str = util.toDateString(Date.from(Instant.now()));// ls nguyenan
            String filename = "His_Log_Game/HisGiaoDich/GiaoDich_" + dateFormatFile.format(date) + ".txt";
            FileWriter fw = new FileWriter(filename, true);
            BufferedWriter bw = new BufferedWriter(fw);
            bw.write("Name : " + name1 + " TradeWith " + name2 + " - Time : " + str + " - ItemTrade : [" + itemsend + "] CoinTrade :" + xusend + " - ItemReceive :[" + itemadd + "] CoinReceive :" + xuadd + "\n");
            bw.close();
            fw.close();
        } catch (Exception e) {
            System.out.println(e);
